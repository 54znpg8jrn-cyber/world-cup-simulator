...
export default function NationRankerPage() {
  ...
  return <main className="stadium-bg min-h-dvh w-full max-w-full overflow-x-hidden px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))] text-white sm:px-5 sm:py-5">
    <div className="mx-auto w-full max-w-3xl min-w-0">
      ...
      <section className="mt-4 rounded-[1.75rem] border border-violet-300/25 bg-[linear-gradient(135deg,rgba(57,39,125,.9),rgba(14,18,58,.96))] p-4 shadow-[0_24px_70px_rgba(0,0,0,.35)] sm:mt-5 sm:p-6"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-violet-200">{mode === "best" ? "Rank highest to lowest" : "Rank lowest to highest"}</p><h2 className="mt-2 text-2xl font-black tracking-[-0.04em] sm:text-3xl">{round.category.label}</h2><p className="mt-1 text-sm text-white/60">{round.category.description}</p><p className="mt-4 text-xs font-bold text-[#f6dc86]">Drag cards on desktop, or use the arrows on any device.</p>
        <div className="mt-4 grid gap-2">{order.map((nation, index) => { const correctIndex = correctOrder.indexOf(nation); const matches = submitted && index === correctIndex; return <article key={nation} draggable={!submitted} onDragStart={() => setDraggedNation(nation)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropNation(nation)} className={`flex min-w-0 items-center gap-2 rounded-2xl border p-3 transition ${submitted ? matches ? "border-emerald-300/35 bg-emerald-300/[0.12]" : "border-rose-300/30 bg-rose-300/[0.08]" : "border-white/10 bg-black/20 hover:border-violet-200/35"}`}><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-white/10 text-xs font-black text-white/65">{index + 1}</span><span className="text-xl">{getNationFlag(nation)}</span><p className="min-w-0 flex-1 truncate text-sm font-black">{nation}</p>{submitted ? <span className="text-right text-xs font-black text-[#f6dc86]">#{correctIndex + 1}<span className="block text-[9px] font-bold text-white/45">{round.category.values[nation]}</span></span> : <div className="flex shrink-0 gap-1"><button type="button" aria-label={`Move ${nation} up`} onClick={() => moveNation(nation, -1)} disabled={index === 0} className="grid h-10 w-10 place-items-center rounded-xl border border-white/10 bg-white/5 text-sm font-black disabled:opacity-25">↑</button><button type="button" aria-label={`Move ${nation} down`} onClick={() => moveNation(nation, 1)} disabled={index === order.length - 1} className="grid h-10 w-10 place-items-center rounded-xl border border-white/10 bg-white/5 text-sm font-black disabled:opacity-25">↓</button></div>}</article>; })}</div>
        {submitted ? <div className="mt-5 text-center"><p className={`text-2xl font-black ${perfect ? "text-emerald-200" : "text-[#f6dc86]"}`}>{perfect ? "Perfect ranking!" : `${roundScore} points`}</p><p className="mt-1 text-sm font-bold text-white/55">{perfect ? "Your perfect streak grows." : "The correct order is now revealed."}</p><div className="mx-auto mt-4 grid max-w-md gap-2 sm:grid-cols-2"><button type="button" onClick={() => startRound()} className="min-h-12 rounded-xl bg-[#d8b75b] px-4 text-xs font-black uppercase tracking-wider text-black">Next Round</button><button type="button" onClick={() => void share()} className="min-h-12 rounded-xl border border-white/15 bg-white/5 px-4 text-xs font-black uppercase tracking-wider text-white/80">Share Score</button></div></div> : <button type="button" onClick={submit} className="mt-5 flex min-h-13 w-full items-center justify-center rounded-xl bg-[#d8b75b] px-4 text-sm font-black uppercase tracking-wider text-black transition hover:bg-[#f6dc86] active:scale-[.98]">Submit Ranking</button>}
